    ...
    //storing command option
     char command[10];
	    ...
	    command[9] = '\0';
			...
			if(data[com_num].endind < data[com_num].stind)
				com_num++;
                        break;
                    }
		    if(j == argc-2)
...
//	struct fdmap fddata[fdcount];
	...
	;int retval;
	    ...
	    if(rc == 0)
	    {
		retval = dup2(fddata[atoi(copy[(data[comind].stind)])],0);
//		printf("File descriptor: %d\n", fddata[atoi(copy[(data[i].stind)])]);
		if(retval < 0)
		    fprintf(stderr, "Error duplicating to file descriptor 0\n"); 
		retval = dup2(fddata[atoi(copy[(data[comind].stind)+1])],1);

		if(retval < 0)
		    fprintf(stderr, "Error duplicating to file descriptor 1\n");
		retval = dup2(fddata[atoi(copy[(data[comind].stind)+2])],2);
