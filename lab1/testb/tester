...
rm -rf dir_test26
mkdir dir_test26
# Test case 26 --chdir, part 1: easy case.
echo ""
echo "--->test case 26:"
./simpsh --creat --rdwr test26io.txt --chdir dir_test26 --creat --rdwr \
  test26io.txt --command 0 1 0 /usr/local/cs/bin/pwd --wait >c26out.txt 2>c26err.txt
if [ $? == 0 ] && [ -e test26io.txt ] && [ ! -s test26io.txt ] && [ ! -s c26err.txt ] \
  && grep -q "exit 0 /usr/local/cs/bin/pwd" c26out.txt \
  && grep -q "$CURRENT_DIR" ./dir_test26/test26io.txt \
  && grep -q "dir_test26" ./dir_test26/test26io.txt
then
  NUM_PASSED=`expr $NUM_PASSED + 1`
...
rm -rf dir_test27
mkdir dir_test27
# Test case 27 --chdir, part 2: complicated case.
echo ""
echo "--->test case 27:"
./simpsh --creat --rdwr test27io.txt --chdir dir_test27 --creat --rdwr \
  test27io.txt --command 0 1 0 /usr/local/cs/bin/pwd --chdir dir_test27 --chdir .. --creat \
  --wronly test27out.txt --command 0 2 0 /usr/local/cs/bin/pwd >c27out.txt 2>c27err.txt
T27RC=$?
sleep 1
if [ $T27RC == 1 ] && [ -e test27io.txt ] && [ ! -s test27io.txt ] && [ -s c27err.txt ] \
  && [ ! -s c27out.txt ] && grep -q "$CURRENT_DIR" ./dir_test27/test27io.txt \
  && grep -q "dir_test27" ./dir_test27/test27io.txt && [ -e test27out.txt ] \
  && [ "$CURRENT_DIR" == "$(cat test27out.txt)" ]
then
  NUM_PASSED=`expr $NUM_PASSED + 1`
