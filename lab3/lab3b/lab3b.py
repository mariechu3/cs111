...
def blockconsist():
	...
	for i in range(12,27): #i blocks
	    ...
	    if(inode[i] in blocks):#duplicate blocks
  	    ...
  	    if(inode[i] in freeblocks):#checks if on free list
		    ...
		    pass
		elif(inode[i] not in alreadyprinted and inode[i] != 0):
	...
	if(entry[5] in freeblocks):#checks if on free list
	    if(entry[5] in blocks and blocks[entry[5]][0] == "True"):
		pass
	    elif(entry[5] not in alreadyprinted and entry[5] != 0):
	        sys.stdout.write("ALLOCATED BLOCK "+entry[5]+" ON FREELIST\n");
	        invalid = True;
	 	alreadyprinted.append(entry[5])
...
def directoryconsist():
	...
	if(entry[6] == "'..'"):
	    if(int(entry[1]) == 2):
		if(str(entry[1]) != str(entry[3])): #check this for the root dir
                    sys.stdout.write("DIRECTORY INODE "+entry[1]+" NAME '..' LINK TO INODE "+entry[3]+" SHOULD BE "+entry[1]+"\n");
                    invalid = True;
		continue;
	    for check in dirent:
		if(check[3] == entry[1] and check[6] != "'.'" and check[6] != "'..'" and entry[3] != check[1]):
		    sys.stdout.write("DIRECTORY INODE "+entry[1]+" NAME '..' LINK TO INODE "+entry[3]+" SHOULD BE "+check[1]+"\n");
                    invalid = True;
		    break;
