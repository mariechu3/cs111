...
{    
    struct ext2_dir_entry* entry;// = (struct ext2_dir_entry*)inode->i_block;
    __u32 size = 0;
    ...
    size = 0;
    while(size < inode->i_size && entry->file_type != 0)
    {
	if(!memcpy(file_name,entry->name,entry->name_len))
	    errorprocess("error in copying name");
	file_name[entry->name_len] = '\0';
        printf("This is reclen: %u\n", entry->rec_len);
        if(entry->inode != 0)
	    printf("DIRENT,%u,%u,%u,%u,%u,'%s'\n",parent,size,entry->inode,entry->rec_len,entry->name_len,file_name);
    ...
    /*struct ext2_dir* entry;
    char [blocksize] block;
    for(int i = 0; i < 12; i++)
    {
        block = inode.iblock[i];
        int size = 0;
    ...
    printf("DIRENT,%u,%u,%u,%u,%u,\"%s\"", parent,byteoffset,,entrylength,namelength,name);
    }*/
}
/*
void indirectblock()
